        ...
        <div className="user-list">
          ...
          {!loading && !error && users.map((u, index) => (
            <div key={index} className={`user-row ${selectedUserId === u.id ? 'active' : ''}`}>
              <div><span>{u.name} {u.surname}</span></div>
              <div><span>Total: {u.totalProjects} / Završeni: {u.completedProjects}</span></div>
              <div><span>Zadnji zavrseni projekat: {u.lastCompletedProjectName ?? 'N/A'}</span></div>
